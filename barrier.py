...
class Barriers:
    barr_img = [pg.transform.rotozoom(pg.image.load(f'image/block.png'), 0, 0.5)]
    def __init__(self, game):
        self.game = game
        self.alien_fleet = game.alien_fleet
        #barrier = Barrier(self.game, img_list=Barriers.barr_img, self.ul, self.wh)
        #for n in range(5):
            #create_barrier()
...
class Barrier(Sprite):
    img_list = [pg.image.load(f'image/block.png')]

    def __init__(self, game, img_list, ul, wh):
        self.game = game
        self.barrier_elements = Group()
        self.image = pg.image.load(f'image/block.png')

...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'image/alien_exp{n}.png') for n in range(4)]
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'image/alien{m}_{n}.png'), 0, 1) for n in range(2)] for m
                    in range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'image/alienship1_{n}.png'), 0, 1) for n in range(2)]
...
class Alien(Sprite):
    def __init__(self, game, image_list, alien_index, sound, start_index=0, ul=(0, 100), v=Vector(1, 0),
        ...
        self.image = pg.image.load('image/alien0_0.png')
        self.screen_rect = self.screen.get_rect()
